...
function UpdateStudent() {
  ...
  const getDetails = async () => {
    ...
    setFatherOccu(data.fatherOccpation);
    setMobile(data.mobile);
    setMotherMobile(data.motherMobile);
    setMotherName(data.motherName);
    setMotherOccu(data.motherOccupation);
    setNationality(data.nationality);
    setStdCategory(data.category);
    setStdClass(data.class);
  ...
  const handleSave = async () => {
    if (
      ...
      await axios.put(`http://localhost:3001/student/${id}`, data);
...
const UpdateStdStyle = styled.div`
  width: 100vw;
  height: 100vh;
  display: flex;
  align-items: center;
